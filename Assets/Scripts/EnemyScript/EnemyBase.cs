...
public class EnemyBase : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            // Default state, move towards patrolling.
            case UnitState.Waiting:
                coroutineStarted = false;
                CurrentUnitState = UnitState.Patrolling;
                Patrol();
                break;
            // When patrolling, we check the line of sight in front of the enemy and the enemy rotates towards its destination.
            case UnitState.Patrolling:
                ...
                RotateTowardsTarget();
                break;
            // Once in the chase state, we keep checking line of sight and rotation as we do in patrolling, but also start the chaseplayer coroutine which makes the enemy pursue the player instead.
        ...
        // If the player is in line of sight, and we aren't chasing yet, we switch to the chase state.
    ...
    // The lock on method for aiming at the player. Once it reaches a threshold, it will start firing. This is affected by the stats of the enemy.
    ...
    // Gathers the enemy properties from the ScriptableObject containing the information.
    ...
    // Increases the stats of the enemy based on the growth rates determined in the SO file. For each level given to the enemy, these stats will grow.
    ...
    // Walk around the map, gather all tiles in the grid and pick one at random. Start pathfinding towards it from current position.
    private void Patrol()
    {
    ...
    // Move through the current list of nodes in CurrentPath generated by the A* Algorithm and cached in Grid.cs.
    ...
    // Uses GetPlayerTile to get the nearest tile to the player and move to it.
    ...
    // Performs a distance check, and returns the tile closest to the enemy.
    ...
    // Returns the tile closest to the player.
    void GetPlayerTile()
    {
        Transform closestTile = null;
        float closestDistance = Mathf.Infinity;
    ...
    // Based on the growth rates determined in the scriptable object.
    ...
    // Basic shooting towards the player.
    ...
    // Performs a check in a radius around the enemy. Lower values will have the enemy only be able to see in front of them, whereas higher values such as 360 will let the enemy always notice the player when in proximity.
    // Passes in a layerMask to save on performance, since it is constantly checked. We only want to check for the player!
